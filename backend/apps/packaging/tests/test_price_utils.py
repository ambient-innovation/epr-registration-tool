...
class PriceServiceTestCase(TestCase):
    ...
    def test_get_price_happy_path(self):
        ...
        self.assertEqual(
            ...
            get_price(material_id=self.material_1.id, year=2021, month=1),
        )
        self.assertEqual(
            self.material_price_1,
            get_price(material_id=self.material_1.id, year=2021, month=1),
        ...
        self.assertEqual(
            self.material_price_1,
            get_price(material_id=self.material_1.id, year=2021, month=5),
        )
        self.assertEqual(
            self.material_price_2,
            get_price(material_id=self.material_1.id, year=2021, month=6),
        )
        self.assertEqual(
            self.material_price_3,
            get_price(material_id=self.material_1.id, year=3000, month=1),
        )

    def test_get_price_with_non_existing_material(self):
        self.assertEqual(None, get_price(material_id=0, year=2021, month=1))

    def test_get_price_with_material_without_any_prices(self):
        self.assertEqual(None, get_price(material_id=self.material_2.id, year=2021, month=1))

    def test_price_override(self):
        self.assertEqual(
            self.material_price_1,
            get_price(material_id=self.material_1.id, year=2021, month=2),
        )
        ...
        self.assertEqual(
            new_price_2,
            get_price(material_id=self.material_1.id, year=2024, month=1),
        )
